...
const CiannCollaboratorsModal = ({ ciannData, onClose, onUpdate }) => {
  ...
  const fetchCollaborators = async () => {
    ...
    setError("");
    try {
      const response = await ciannUtils.getCiannShares(ciannData.ciannId);
  ...
  const fetchLogs = async () => {
    ...
    setError("");
    try {
      const response = await ciannUtils.fetchCollaborationLogs(ciannData.ciannId);
  ...
  }, [activeSubTab, ciannData.ciannId]);
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(async () => {
      ...
      try {
        const results = await ciannUtils.fetchFacultyDirectory(searchQuery.trim());
  ...
  const handleShareDirect = async (e) => {
    ...
    try {
      const result = await ciannUtils.shareCiann(
        ciannData.ciannId,
  ...
  const handleUpdatePermission = async (userId, username, newPermission) => {
    ...
    setSuccessMsg("");
    try {
      const result = await ciannUtils.shareCiann(ciannData.ciannId, username, newPermission);
  ...
  const handleRevokeAccess = async (userId, username) => {
    ...
    setSuccessMsg("");
    try {
      await ciannUtils.removeCiannShare(ciannData.ciannId, userId);
  ...
  return (
    <div className="ciann-modal-backdrop" onClick={onClose}>
      <div className="ciann-modal-dialog modal-lg" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-header bg-dark text-white border-0 py-3 px-4 d-flex justify-content-between align-items-center" style={{ borderTopLeftRadius: "16px", borderTopRightRadius: "16px" }}>
            <div>
              <h5 className="modal-title fw-bold mb-0">
                ...
                CIANN Collaboration Panel
              </h5>
              <small className="text-light opacity-75">
                CIANN ID: {ciannData.ciannId} | Subject: {ciannData.subject?.name} ({ciannData.subject?.code})
              </small>
            </div>
                        ...
                        <tr>
                          <td>
                            <div className="d-flex align-items-center">
                              ...
                              <div>
                                <span className="fw-bold text-dark">{ciannData.ownerUsername || ciannData.owner?.username}</span>
                                <span className="badge bg-success ms-2">Primary Owner</span>
                              </div>
                            </div>
                          </td>
                          <td><span className="text-capitalize text-muted">{ciannData.ownerRole || ciannData.owner?.role || "Faculty"}</span></td>
                          <td><span className="text-muted">{ciannData.college || "N/A"}</span></td>
...
export default CiannCollaboratorsModal;
