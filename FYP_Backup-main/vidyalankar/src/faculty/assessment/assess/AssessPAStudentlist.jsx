  ...
  const fetchStudents = async () => {
    try {
      ...
      if (isEditMode && experiment) {
        ...
        try {
          // First try the regular students endpoint - filter by both batch and division from Ciaan
        ...
        if (studentsData.length === 0) {
          const division = CiaanData?.division || "Unknown";
          console.warn(
  ...
  const handleSubmit = async () => {
    try {
      ...
      const requestPayload = {
        ...
        program: program || CiaanData?.department?.name || "",
        className: className || CiaanData?.class || CiaanData?.division || "",
        course:
          course || CiaanData?.subject?.name || CiaanData?.subject?.code || "",
                ...
                {filteredStudents.length === 0 ? (
                  <tr>
                    <td colSpan="3" className="text-center">
                      {students.length === 0
                        ? `No students found for this batch (${CiaanData?.division || "Unknown"} division)`
