...
const K7CiannSelection = () => {
  ...
  const handleCiannClick = (ciann) => {
    ...
    navigate(
      `/msbte/k7/generate?ciannId=${ciann.ciannId}&academicYear=${academicYear}&semester=${semester}&departmentId=${deptId}&divisionName=${divisionName}&courseCode=${courseCode}`
    );
  };
  ...
  const filteredCianns = cianns.filter(c => {
    ...
    const matchesSearch = (
      c.ciannId.toString().includes(query) ||
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
          <div>
            ...
            <p style={{ margin: "4px 0 0", color: "#64748b" }}>
              Select a class/subject (CIANN) to manage its Term End Examination result analysis.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-5 card" style={{ background: "#ffffff", borderRadius: "14px", padding: "40px" }}>
            ...
            <h5 className="mt-3">No CIANNs Found</h5>
            <p className="text-muted">No classes matching your search or no CIANNs are assigned to you.</p>
          </div>
        ) : (
          <div className="row g-3">
            {filteredCianns.map((c) => (
              <div key={c._id} className="col-md-4">
                <div
                  className="card h-100 ciann-selection-card"
                  onClick={() => handleCiannClick(c)}
                  style={{
                    background: "#ffffff",
...
export default K7CiannSelection;
