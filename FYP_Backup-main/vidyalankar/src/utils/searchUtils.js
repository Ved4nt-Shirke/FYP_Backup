    ...
    try {
      ...
      const theoryResults = theoryResponse.data
        .filter(record => 
          ...
          record.ciannId.toLowerCase().includes(searchQuery) ||
        ...
        .map(record => ({
          ...
          title: record.topic,
          subtitle: `CIANN ID: ${record.ciannId} | Date: ${record.date}`,
    ...
    try {
      ...
      const tutorialResults = tutorialResponse.data
        ...
        .map(record => ({
          ...
          title: record.Topic,
          subtitle: `CIANN ID: ${record.ciannId} | Date: ${record.actualDate}`,
  ...
  switch (result.type) {
    ...
    case 'attendance':
      ...
      if (result.subtype === 'theory') {
        navigate('/theory-ciann-cards');
      } else if (result.subtype === 'practical') {
        navigate('/practical-ciann-cards');
