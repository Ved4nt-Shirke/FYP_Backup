  ...
  try {
    ...
    try {
      ...
      const notifications = facultyUsers.map((user) => ({
        ...
        message: `Academic Year ${year.yearName} has been completed/archived. CIANN editing for this year is now read-only.`,
      }));
...
// Get CIANN tracking stats for a specific academic year.
// Admin only.
// ──────────────────────────────────────────────
router.get("/:id/stats", authorizeAdmin, async (req, res) => {
  try {
    ...
    // Get all CIANNs for this academic year (matching by yearName string)
    const cianns = await Ciann.find({
      college: year.college,
      academicYear: year.yearName,
    }).populate("faculty", "fullName department");

    const totalCianns = cianns.length;
    const completedCianns = cianns.filter((c) => c.status === "completed").length;
    const activeCianns = cianns.filter((c) => c.status === "active").length;
    const archivedCianns = cianns.filter((c) => c.status === "archived").length;

    // Faculty-wise stats
    const facultyMap = {};
    cianns.forEach((ciann) => {
      const ownerKey = ciann.ownerUsername || ciann.owner?.toString() || "Unknown";
      ...
      if (ciann.status === "completed") facultyMap[ownerKey].completed += 1;
      else if (ciann.status === "active") facultyMap[ownerKey].active += 1;
      else if (ciann.status === "archived") facultyMap[ownerKey].archived += 1;
    });
    ...
    const deptMap = {};
    cianns.forEach((ciann) => {
      const deptName = ciann.department?.name || "Unknown";
      if (!deptMap[deptName]) {
        deptMap[deptName] = { department: deptName, total: 0, completed: 0, pending: 0 };
      }
      deptMap[deptName].total += 1;
      if (ciann.status === "completed") deptMap[deptName].completed += 1;
    ...
    const semesterMap = {};
    cianns.forEach((ciann) => {
      const sem = ciann.semester || "Unknown";
      if (!semesterMap[sem]) {
        semesterMap[sem] = { semester: sem, total: 0, completed: 0, pending: 0 };
      }
      semesterMap[sem].total += 1;
      if (ciann.status === "completed") semesterMap[sem].completed += 1;
    ...
    return res.json({
      ...
      stats: {
        ...
        activeCianns,
        archivedCianns,
        completionPercentage: totalCianns > 0 ? Math.round((completedCianns / totalCianns) * 100) : 0,
        facultyStats,
        departmentStats,
        subjectStats,
        semesterStats,
