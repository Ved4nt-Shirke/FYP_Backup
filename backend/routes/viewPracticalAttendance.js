...
/**
 * @route   GET /api/view-practical-attendance/:ciannId
 * @desc    Get aggregated practical attendance data for a specific CIAAN ID organized by batch.
 * @access  Public
 */
router.get('/:ciannId', async (req, res) => {
  try {
    console.log('GET /api/view-practical-attendance/:ciannId - Request params:', req.params);
    ...
    console.log('Searching for practical attendance records with ciannId:', ciannIdAsNumber);
    const practicalAttendanceRecords = await PracticalAttendance.find({ ciannId: ciannIdAsNumber }).lean();
    ...
    if (!practicalAttendanceRecords || practicalAttendanceRecords.length === 0) {
      console.log('No practical attendance records found for CIAAN ID:', ciannIdAsNumber);
...
/**
 * @route   GET /api/view-practical-attendance/:ciannId/:batch
