...
router.post("/", async (req, res) => {
  const { topic, date, ciannId } = req.body;
  try {
    const newDoc = new ExtraAttendance({ topic, date, ciannId, students: [] });
    await newDoc.save();
...
// Get all extra attendance records by ciannId (MUST come before /:id route)
router.get('/ciann/:ciannId', async (req, res) => {
  try {
    const { ciannId } = req.params;
    const records = await ExtraAttendance.find({ ciannId: parseInt(ciannId) });
    res.json(records);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
router.put('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { topic, date, students, ciannId } = req.body;

    const updateData = { topic, date, students, ciannId };
