...
router.post("/promotions/promote", authenticate, authorizeOffice, async (req, res) => {
  try {
    ...
    // 4. Lock/Archive Ciaan records for the source division of the old semester
    ...
    if (sourceDivision) {
      ...
      if (oldAcademicYear) {
        query.academicYear = oldAcademicYear; // FIX: Sync specific academic year Ciaans!
      }

      const CiaanUpdateResult = await Ciaan.updateMany(
        query,
        { $set: { status: "completed" } }
      );
      console.log(`[PROMOTION] Locked Ciaan records for division ${sourceDivision.name}, sem ${sourceSemester}. Count:`, CiaanUpdateResult.modifiedCount);
    }
...
router.get("/archive/semesters", authenticate, authorizeOffice, async (req, res) => {
  try {
    ...
    // Get subjects in this department to filter Ciaan records
    ...
    // Retrieve Ciaan records matching these subjects
    ...
    Ciaans.forEach((c) => {
      ...
      groupsMap[key].totalCiaans++;
      if (c.status === "completed") {
        groupsMap[key].completedCiaans++;
      }
    });

    const semesters = Object.values(groupsMap).map((g) => ({
      ...g,
      status: g.completedCiaans === g.totalCiaans ? "completed" : g.completedCiaans > 0 ? "partial" : "active",
    }));
...
// Freeze or unfreeze all Ciaan/course-diary edits for a particular semester
router.post("/archive/freeze", authenticate, authorizeOffice, async (req, res) => {
  try {
    ...
    const result = await Ciaan.updateMany(
      {
        college: institution,
        academicYear,
        division,
