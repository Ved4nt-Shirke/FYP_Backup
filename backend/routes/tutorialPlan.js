...
// GET tutorial plans for a specific CiaanId
router.get('/:CiaanId', async (req, res) => {
  try {
    ...
    const plans = await TutorialPlan.find({ CiaanId: numericCiaanId }).lean();
...
router.post('/', checkCiaanFreeze, async (req, res) => {
  try {
    ...
    const newPlan = new TutorialPlan({ CiaanId, weekNo, plans });
    await newPlan.save();
...
// PUT to update or create a tutorial plan by CiaanId and weekNo
