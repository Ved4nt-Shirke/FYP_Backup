...
// Get unique divisions for the faculty's batch/Ciaan from office staff data
router.get("/divisions", async (req, res) => {
  try {
    ...
    // Get the Ciaan to find the batch
    const ciaanDoc = await Ciaan.findById(batchId);
    if (!ciaanDoc) {
    ...
    // Get all Ciaans with the same batch/class to find all divisions
    const allCiaansInBatch = await Ciaan.find({
      class: ciaanDoc.class,
      college: ciaanDoc.college,
    }).select("division");

    // Extract unique divisions from Ciaans
    let divisions = [...new Set(allCiaansInBatch.map((c) => c.division).filter((d) => d))];

    // If no divisions found in Ciaans, try Division collection
...
router.get("/submissions/available-exams", authenticate, async (req, res) => {
  try {
    ...
    if (!student) {
      console.log("Student not found by username, trying to find from Ciaans");
