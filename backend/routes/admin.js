...
router.get("/archive/semesters", authenticate, authorizeAdmin, async (req, res) => {
  try {
    const institution = req.user.college;
    const Ciaans = await Ciaan.find({ college: institution })
...
// Freeze, unfreeze, or archive Ciaan documents for a specific semester
router.post("/archive/freeze", authenticate, authorizeAdmin, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Successfully updated status to '${status}' for ${result.modifiedCount} Ciaan sheets.`,
