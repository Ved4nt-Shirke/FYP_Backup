...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const [departmentDocs, courseDocs, divisionDocs, userDocs, CiaanDocs] =
        await Promise.all([
          ...
          Ciaan.find({ college: institutionCode }).select("CiaanId"),
        ]);
      ...
      const CiaanIds = CiaanDocs.map((doc) => doc.CiaanId);
      ...
      const deleteByCiaanIds = async (Model) => {
        if (CiaanIds.length === 0) {
          return;
        }

        await Model.deleteMany({ CiaanId: { $in: CiaanIds } });
      };
      ...
      await Promise.all([
        deleteByCiaanIds(Assessment),
        deleteByCiaanIds(CTMarks),
        deleteByCiaanIds(TeachingPlan),
        deleteByCiaanIds(TheoryAttendance),
        deleteByCiaanIds(PracticalAttendance),
        deleteByCiaanIds(TutorialAttendance),
        deleteByCiaanIds(ExtraAttendance),
        deleteByCiaanIds(ExtraPract),
        deleteByCiaanIds(KnowledgeMap),
        deleteByCiaanIds(LabPlanning),
        deleteByCiaanIds(MoocCourse),
        deleteByCiaanIds(CourseOutcome),
        deleteByCiaanIds(SubjectObjective),
        deleteByCiaanIds(BookResource),
        deleteByCiaanIds(WebResource),
      ]);
      ...
      await Promise.all([
        ...
        Ciaan.deleteMany({ college: institutionCode }),
