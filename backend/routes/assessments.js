...
const resolveDivisionFromQuery = async ({ CiaanId, division }) => {
  const normalizedDivision = String(division || '').trim();
  if (normalizedDivision) return normalizedDivision;

  const numericCiaanId = parseInt(CiaanId, 10);
  if (!Number.isNaN(numericCiaanId)) {
    const ciaanDoc = await Ciaan.findOne({ CiaanId: numericCiaanId }).select('division');
    return String(ciaanDoc?.division || '').trim();
  }
...
// GET distinct CiaanIds that have at least one saved assessment
router.get('/assessed-Ciaans', async (req, res) => {
  try {
    const distinctCiaanIds = await Assessment.distinct('CiaanId', {
      CiaanId: { $exists: true, $ne: null }
    });
    res.json({ success: true, CiaanIds: distinctCiaanIds.filter(id => id != null) });
  } catch (error) {
    console.error('Error fetching assessed Ciaans:', error);
    res.status(500).json({ success: false, message: 'Server error' });
  }
});
...
router.get('/assessed-experiments', async (req, res) => {
  try {
    ...
    if (batch) {
      ...
      if (studentNames.length === 0) {
        return res.json({ success: true, experiments: [], CiaanIds: [] });
      }
    ...
    if (CiaanId) {
      // Support both numeric and string CiaanId stored in assessments
      const numericId = Number(CiaanId);
      if (!isNaN(numericId)) {
        matchQuery.$or = [{ CiaanId: numericId }, { CiaanId: CiaanId }];
      } else {
        matchQuery.CiaanId = CiaanId;
      }
    }

    // Get distinct experiment numbers and names from assessments for this batch and Ciaan
    ...
    // Also return distinct CiaanIds that have assessments (for Ciaan filtering)
    let assessedCiaanIds = [];
    if (!CiaanId) {
      const CiaanIdResults = await Assessment.distinct('CiaanId', batch ? matchQuery : {});
      assessedCiaanIds = CiaanIdResults.filter(Boolean);
    }

    res.json({ success: true, experiments: assessedExperiments, CiaanIds: assessedCiaanIds });
  } catch (error) {
...
router.get('/students-by-batch', async (req, res) => {
  try {
    const { batch, CiaanId, division } = req.query;
    ...
    const resolvedDivision = await resolveDivisionFromQuery({ CiaanId, division });
