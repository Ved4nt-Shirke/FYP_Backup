 ...
 * FLOW:
 *  Faculty: "hi"
 *  Bot: Greets + shows Ciaan list
 *  Faculty: replies with Ciaan number (e.g. "1234")
 ...
 *  Faculty: ATT | CiaanId | date | topic\nP: 1,3\nA: 2,4
...
client.on("message", async (message) => {
    ...
    switch (session.state) {
        case "AWAIT_Ciaan":
            await handleCiaanSelect(message, raw, faculty, phone, session);
            break;
        case "AWAIT_TYPE":
...
// ── Step 1: Greeting + Show Ciaans ─────────────────────────────────────────
async function handleGreeting(message, faculty, phone) {
    try {
        const Ciaan = getModel("Ciaan");
        ...
        Ciaans.forEach((c, i) => {
            ...
            reply += `   Ciaan ID: *${c.CiaanId}* | Div: ${c.division} | Sem ${c.semester}\n\n`;
        });

        reply += `━━━━━━━━━━━━━━━━━━\n`;
        reply += `Reply with the *Ciaan ID* (e.g. *${Ciaans[0].CiaanId}*) to continue.\n`;
        reply += `\n_Send "cancel" anytime to reset._`;

        setSession(phone, { state: "AWAIT_Ciaan", Ciaans, faculty });
        await message.reply(reply);
    } catch (err) {
        console.error("handleGreeting error:", err);
        await message.reply("❌ Error loading your Ciaans. Try again later.");
    }
}

// ── Step 2: Faculty selects a Ciaan ────────────────────────────────────────
async function handleCiaanSelect(message, raw, faculty, phone, session) {
    const CiaanId = parseInt(raw);
    if (isNaN(CiaanId)) {
        await message.reply("❓ Please reply with a valid *Ciaan ID* (4-digit number).\nOr send *hi* to see your list again.");
        return;
    }

    const Ciaan = session.Ciaans?.find((c) => c.CiaanId === CiaanId);
    if (!Ciaan) {
        await message.reply(`❌ Ciaan ID *${CiaanId}* not found in your list.\nPlease reply with one of your Ciaan IDs.`);
        return;
    }

    setSession(phone, { ...session, state: "AWAIT_TYPE", selectedCiaan: Ciaan });

    await message.reply(
        `✅ Selected: *${Ciaan.subject?.name}* (${Ciaan.subject?.code})\n` +
        `Division: ${Ciaan.division} | Sem ${Ciaan.semester}\n\n` +
...
async function handleTypeSelect(message, raw, phone, session) {
    ...
    const Ciaan = session.selectedCiaan;
    ...
    try {
        ...
        if (isTheory) {
            ...
            const plans = await TeachingPlan.find({ CiaanId: Ciaan.CiaanId }).lean();

            // Get already marked attendances to check which are done
            const markedRecords = await TheoryAttendance.find({ CiaanId: Ciaan.CiaanId }).select("date chapter subTopic").lean();
            ...
            reply += `📖 *Teaching Plan for Ciaan ${Ciaan.CiaanId}*\n*${Ciaan.subject?.name}*\n━━━━━━━━━━━━━━━━━\n`;
        } else {
            const LabPlanning = getModel("LabPlanning");
            const PracticalAttendance = getModel("PracticalAttendance");
            const plans = await LabPlanning.find({ CiaanId: Ciaan.CiaanId }).lean();

            const markedRecords = await PracticalAttendance.find({ CiaanId: Ciaan.CiaanId }).lean();
            ...
            reply += `🧪 *Lab Plan for Ciaan ${Ciaan.CiaanId}*\n*${Ciaan.subject?.name}*\n━━━━━━━━━━━━━━━━━\n`;
        }
        ...
        if (availablePlans.length > 0) {
            ...
            await message.reply(
                `⚠️ No ${isTheory ? 'Teaching' : 'Lab'} Plan found for Ciaan *${Ciaan.CiaanId}*.\n\n` +
...
async function handleAbsentRolls(message, raw, faculty, phone, session) {
    const { selectedCiaan, selectedPlanDetails, type } = session;
    const CiaanId = selectedCiaan.CiaanId;
    const date = selectedPlanDetails.date;

    const students = await resolveStudents({
        division: selectedCiaan.division,
        academicYear: selectedCiaan.academicYear,
        semester: selectedCiaan.semester
    }, selectedCiaan.college || (faculty && faculty.institution));

    if (students.length === 0) {
        await message.reply(`⚠️ No students found in Division ${selectedCiaan.division}. Add students from the admin panel first.`);
        return;
    }
    ...
    try {
        ...
        await message.reply(
            ...
            `🏷️ Ciaan: ${CiaanId} | Div: ${selectedCiaan.division}\n` +
            `📅 Date: ${date}\n` +
...
async function handleTpDate(message, raw, phone, session) {
    ...
    const { selectedCiaan, newPlan } = session;
    try {
        const TeachingPlan = getModel("TeachingPlan");
        await TeachingPlan.findOneAndUpdate(
            { CiaanId: selectedCiaan.CiaanId, weekNo: 1 },
...
async function handleDirectAttendance(message, body, faculty, phone) {
    ...
    if (isNaN(CiaanId)) { await message.reply("❌ Invalid Ciaan ID."); return; }
    ...
    try {
        const CiaanModel = getModel("Ciaan");
        const Ciaan = await CiaanModel.findOne({ CiaanId }).lean();
        if (!Ciaan) { await message.reply(`❌ Ciaan ${CiaanId} not found.`); return; }

        const TheoryAttendance = getModel("TheoryAttendance");
        const existing = await TheoryAttendance.findOne({ CiaanId, date }).lean();
        if (existing) {
        ...
        const students = await resolveStudents({
            ...
            academicYear: Ciaan.academicYear,
            semester: Ciaan.semester
        }, Ciaan.college);

        if (students.length === 0) {
            await message.reply(`⚠️ No students found in Division ${Ciaan.division}. Add students from the admin panel first.`);
            return;
        }
        ...
        await new TheoryAttendance({
            CiaanId, date, topic: topic || "Not specified", chapter: "", startDate: date,
            teachingMethod: "WhatsApp",
            remark: `Marked via WhatsApp by ${faculty.fullName}`,
            students: attendanceStudents,
        }).save();
        ...
        await message.reply(
            `✅ *Attendance Saved!*\n━━━━━━━━━━━━━━━━━━\n` +
            `📚 ${Ciaan.subject?.name} (${Ciaan.subject?.code})\n` +
            `🏷️ Ciaan: ${CiaanId} | Div: ${Ciaan.division}\n` +
