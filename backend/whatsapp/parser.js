 ...
 *
 * Expected format:
 *   ATT | CiaanId | YYYY-MM-DD | Topic Name
 *   P: 1,3,5,7
 *   A: 2,4,6,8
 */
...
function parseAttendanceMessage(body) {
  ...
  const CiaanId = parseInt(parts[1]);
  if (isNaN(CiaanId) || CiaanId < 1000 || CiaanId > 9999) {
    return {
      valid: false,
      error: `❌ Invalid Ciaan ID "${parts[1]}". Must be a 4-digit number.`,
    };
  }
