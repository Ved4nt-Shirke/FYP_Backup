...
const SmartAttendanceHub = () => {
  ...
  // ── Open modal ───────────────────────────────────────────────────
  const openModal = (ciann) => {
    setSelectedCiann(ciann);
  ...
  const handleTypeSelect = async (type) => {
    ...
    try {
      if (type === "theory") {
        const res = await fetch(`${config.teachingPlan}/${ciann.ciannId}`, {
        ...
        if (hasPlans) {
          ...
          navigate("/theory-edit", { state: { ciannData: ciann } });
          closeModal();
        } else {
        ...
        // Practical
        const res = await fetch(`${config.labPlanning}/${ciann.ciannId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
        const hasPlans =
        ...
        if (hasPlans) {
          navigate("/practical-attendance", { state: { ciannData: ciann } });
          closeModal();
        } else {
  ...
  const handleSaveTeachingPlan = async (e) => {
    ...
    try {
      ...
      navigate("/theory-edit", { state: { ciannData: selectedCiann } });
      closeModal();
    } catch (err) {
    ...
    <>
      ...
      <div className="sah-page">
        <div className="sah-header">
          ...
          <p>
            Select a CIANN to mark Theory or Practical attendance in one place.
          </p>
        </div>

        <div className="sah-grid">
          {loading ? (
            <div className="sah-loading">Loading your CIANNs...</div>
          ) : cianns.length === 0 ? (
            <div className="sah-empty">
              <h3>No CIANNs Found</h3>
              <p>
                You haven't created any CIANNs yet. Go to Create CIANN to get
                started.
              </p>
            </div>
          ) : (
            cianns.map((ciann) => (
              <div
                key={ciann._id}
                className="sah-card"
                onClick={() => openModal(ciann)}
              >
                <div className="sah-card-id">CIANN ID: {ciann.ciannId}</div>
                <div className="sah-card-subject">
                  {ciann.subject?.name || "Unknown Subject"}
                </div>
                <div className="sah-card-code">{ciann.subject?.code || ""}</div>
                <div className="sah-card-meta">
                  <span className="sah-tag">Div: {ciann.division}</span>
                  <span className="sah-tag">Sem {ciann.semester}</span>
      ...
      {selectedCiann && modalState !== "idle" && (
        ...
        >
          <div className="sah-modal">
            ...
            {/* CIANN Info */}
            <div className="sah-modal-ciann-info">
              <p>
                {selectedCiann.subject?.name} ({selectedCiann.subject?.code})
              </p>
              <span>
                CIANN {selectedCiann.ciannId} · Division{" "}
                {selectedCiann.division} · Sem {selectedCiann.semester}
              </span>
            </div>
            ...
            {modalState === "checking" && (
              <div className="sah-checking">
                ⏳ Checking plan details for CIANN {selectedCiann.ciannId}...
              </div>
            )}
