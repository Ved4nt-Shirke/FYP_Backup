...
const EditIndividualPracticalAttendance = () => {
  ...
  const { practicalData, CiaanData, CiaanId } = location.state || {};
  ...
  useEffect(() => {
    if (!practicalData || !CiaanId) {
      setError("Missing practical data or Ciaan ID");
      setLoading(false);
      return;
    }
  ...
  const handleSave = async () => {
    if (!window.confirm("Are you sure you want to save this practical attendance?")) {
      return;
    }
    ...
    try {
      // Prepare attendance data for practical attendance
      const attendanceData = {
        CiaanId: CiaanId,
        weekNo: practicalData.weekNo,
        batch: practicalData.batch,
        exptNo: practicalData.exptNo,
        exptName: practicalData.exptName,
        actualDate: practicalData.actualDate,
        remark: practicalData.remark || "",
      ...
      navigate("/edit-practical-attendance2", { state: { selectedCiaanId: CiaanId } });
    } catch (error) {
    ...
    return (
      <div className="error-container">
        <div className="error-message-box">
          ...
          <div style={{ marginBottom: '20px' }}>No practical data provided</div>
          <button className="button-base button-secondary" onClick={handleCancel}>
            Go Back
          </button>
        </div>
...
export default EditIndividualPracticalAttendance;
