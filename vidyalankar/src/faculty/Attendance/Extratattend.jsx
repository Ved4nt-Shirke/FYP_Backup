  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch students filtered by division if available from CIANN
        const params = new URLSearchParams();
        if (attendanceData.ciannId) {
          ...
          try {
            ...
            if (ciannResponse.ok) {
              ...
              const ciann = cianns.find(
                (c) => c.ciannId === attendanceData.ciannId,
              );
              if (ciann?.division) {
                params.append("division", ciann.division);
              }
            }
          } catch (ciannError) {
            console.warn(
              "Could not fetch CIANN data for filtering:",
              ciannError,
            );
          }
        }
        ...
        {attendanceRecord && (
          ...
          >
            ...
            <p style={{ margin: "5px 0", fontSize: "14px" }}>
              <strong>CIANN ID:</strong> {attendanceRecord.ciannId}
            </p>
            <p style={{ margin: "5px 0", fontSize: "14px" }}>
