...
const ViewPractical2 = () => {
    ...
    const fetchPracticalAttendanceData = async () => {
      try {
        const response = await axios.get(
          `${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5000").replace(/\/api$/, "")}/api/view-practical-attendance/${CiaanId}`,
        );
  ...
  const handleSubmit = () => {
    ...
    const CiaanId = searchParams.get("CiaanId");
    window.open(
      `/view-practical3?CiaanId=${CiaanId}&batch=${selectedBatch}`,
      "_blank",
    );
  };
  ...
  if (
    ...
    return (
      <div className="vbs-state">
        No practical attendance data found for this Ciaan.
      </div>
    );
  }
