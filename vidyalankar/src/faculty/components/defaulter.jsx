...
const Defaulter = () => {
      ...
      try {
        const studentsRes = await fetchApi(
          `${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5000").replace(/\/api$/, "")}/api/students?${new URLSearchParams({
            ...(selectedCiaan.divisionId ? { divisionId: selectedCiaan.divisionId } : {}),
            ...(selectedCiaan.division && !selectedCiaan.divisionId ? { division: selectedCiaan.division } : {}),
            ...(selectedCiaan.academicYear ? { academicYear: selectedCiaan.academicYear } : {}),
          }).toString()}`,
        );
        ...
        const practicalRes = await fetchApi(
          `${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5000").replace(/\/api$/, "")}/api/practical-attendance?CiaanId=${CiaanId}`,
        );
        ...
        const extraPractRes = await fetchApi(
          `${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5000").replace(/\/api$/, "")}/api/extra-pract?CiaanId=${CiaanId}`,
        );
  ...
  const attendanceSummary =
    selectedCiaan && attendanceData ? processAttendance() : {};
      ...
      <div className="defaulter-page-container">
        ...
        <div className="defaulter-select-page">
          ...
          {!loading && !error && Ciaans.length === 0 && (
            <p className="defaulter-state">No Ciaans found.</p>
          )}

          {!loading && !error && Ciaans.length > 0 && (
            <div className="defaulter-Ciaan-grid">
              {Ciaans.map((Ciaan) => (
                <button
                  key={Ciaan._id || Ciaan.CiaanId}
                  type="button"
                  className="defaulter-Ciaan-card"
    ...
    <div className="defaulter-page-container">
      ...
      <div className="defaulter-detail-page">
        ...
        <div className="print-only-header" style={{ display: "none" }}>
          <div style={{ textAlign: "center", marginBottom: "20px" }}>
            ...
            <p style={{ margin: "5px 0", fontSize: "12pt" }}>
              Subject: {selectedCiaan.subject.name} | Academic Year:{" "}
              {selectedCiaan.academicYear} | Semester: {selectedCiaan.semester}
            </p>
        ...
        <div className="defaulter-header">
          <h1>Defaulter Students ({selectedCiaan.subject.name})</h1>
