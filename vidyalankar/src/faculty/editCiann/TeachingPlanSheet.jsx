...
const TeachingPlan = () => {
  ...
  // Fetch theory attendance records for the selected CiaanId
  useEffect(() => {
    if (!CiaanId) return;
    const fetchAttendance = async () => {
      try {
        ...
        const res = await fetch(
          getApiUrl(`/theory-attendance?CiaanId=${CiaanId}`),
          {
            headers: {
  ...
  // Fetch TloLlo details for CO and TLO dropdowns
  useEffect(() => {
    if (!CiaanData) return;
    const fetchTloLlo = async () => {
      try {
        ...
        if (data.success && data.data && Array.isArray(data.data.coData)) {
          ...
          // Fallback to course outcomes from courseDetails endpoint
        ...
        console.error("Error fetching TloLlo mappings in Teaching Plan:", err);
      }
    };
    fetchTloLlo();
  }, [CiaanData]);
  ...
  useEffect(() => {
    const fetchChapters = async () => {
      try {
        const courseName = CiaanData?.subject?.name; // e.g., "Computer Graphics"
        const programLabel = CiaanData?.department?.label; // e.g., "Computer Engineering"
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      if (!CiaanId) {
        console.warn("No CiaanId available, cannot fetch plans");
        setTeachingPlans([]);
      ...
      try {
        const res = await fetch(
          getApiUrl(`/teaching-plan/${CiaanId}`),
        );
        if (res.status === 404) {
          console.log(`No teaching plans found for CiaanId: ${CiaanId}`);
          setTeachingPlans([]);
          setPlanningStarted(false);
          return;
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        getApiUrl(`/teaching-plan/${CiaanId}/${numericWeek}`),
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
                    ...
                    <td className="co-cell">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          const updated = [...plans];
                          updated[index].co = val;
                          updated[index].tlo = ""; // reset tlo when co changes
                          setPlans(updated);
                        }}
                        disabled={!modalWeek}
                ...
                {!planningStarted ? (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setView("add");
                      const nextWeek = teachingPlans.length > 0
                  ...
                  <div className="action-buttons">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setView("edit");
                        const latestWeek = teachingPlans.length > 0
                          ? Math.max(...teachingPlans.map((p) => p.weekNo))
                          : 1;
