...
const TutorialPlanSheet = () => {
  ...
  useEffect(() => {
    ...
    const fetchChapters = async () => {
      try {
        const courseName = ciannData?.subject?.name;
        const programLabel = ciannData?.department?.label;
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      ...
      try {
        ...
        const res = await fetch(
          getApiUrl(`/tutorial-plan/${ciannId}`),
          {
            headers: {
  ...
  useEffect(() => {
    ...
    const fetchAttendance = async () => {
      try {
        const token = localStorage.getItem("token");
        const res = await fetch(
          getApiUrl(`/tutorial-attendance?ciannId=${ciannId}`),
          {
            headers: {
              Authorization: `Bearer ${token}`
            }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        getApiUrl(`/tutorial-plan/${ciannId}/${numericWeek}`),
        {
          method: "PUT",
          headers: { 
