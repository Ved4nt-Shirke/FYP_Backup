...
const getClassAndDiv = (CiaanData) => {
...
const getSubjectAndCode = (CiaanData) => {
  if (!CiaanData) return "N/A";
...
const getDepartment = (CiaanData) => {
  return (
    CiaanData?.department?.label ||
...
const PrintCiaan = () => {
  ...
  const getTeacherNames = () => {
    if (!CiaanData) return formatUsername(localStorage.getItem("username") || "-");
  ...
  useEffect(() => {
    const storedCiaanData = localStorage.getItem("CiaanData");
    if (!storedCiaanData) {
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      console.error("Failed to parse CiaanData for printing:", error);
      setLoading(false);
    }
  }, []);
  ...
  const fetchPrintData = async (data) => {
    ...
    try {
      const responses = await Promise.allSettled([
        ...
        fetch(`${config.subjectDetails}/course-outcomes/${CiaanId}`, {
          headers: requestHeaders,
          credentials: "include",
        }),
        fetch(`${config.subjectDetails}/objectives/${CiaanId}`, {
          headers: requestHeaders,
          credentials: "include",
        }),
        fetch(`${config.subjectDetails}/knowledge-map/${CiaanId}`, {
          headers: requestHeaders,
          credentials: "include",
        }),
        fetch(`${config.subjectDetails}/book-resources/${CiaanId}`, {
          headers: requestHeaders,
          credentials: "include",
        }),
        fetch(`${config.subjectDetails}/web-resources/${CiaanId}`, {
          headers: requestHeaders,
          credentials: "include",
        }),
        fetch(`${config.subjectDetails}/mooc-courses/${CiaanId}`, {
          headers: requestHeaders,
          credentials: "include",
        }),
        fetch(`${config.subjectDetails}/syllabus/${CiaanId}`, {
          headers: requestHeaders,
          credentials: "include",
        }),
      ]);
      ...
      if (Array.isArray(syllabusRes?.images) && syllabusRes.images.length > 0) {
        ...
        const syllabusKey = `syllabusImages:${CiaanId}`;
      ...
      try {
        ...
        if (instVMResponse.ok) {
          ...
          console.warn("[PrintCiaan] Institute Vision/Mission fetch failed:", instVMResponse.status);
        }
      } catch (err) {
        console.error("[PrintCiaan] Error fetching institute vision/mission:", err);
      }
      ...
      try {
        ...
        if (deptVMResponse.ok) {
          ...
          console.warn("[PrintCiaan] Department Vision/Mission fetch failed:", deptVMResponse.status);
        }
      } catch (err) {
        console.error("[PrintCiaan] Error fetching department vision/mission:", err);
      }
      ...
      if (CiaanId && resolvedDivName) {
        try {
          const k4Res = await fetch(
            `${config.msbte}/sa-pr-k4?CiaanId=${encodeURIComponent(CiaanId)}&division=${encodeURIComponent(resolvedDivName)}`,
            { headers: requestHeaders }
          );
          if (k4Res.ok) {
      ...
      if (CiaanId) {
        try {
          const k5Res = await fetch(`${config.apiBaseUrl}/ct-marks/${CiaanId}`, {
            headers: requestHeaders,
          });
          if (k5Res.ok) {
      ...
      try {
        const theoryRes = await fetch(
          `${config.apiBaseUrl}/theory-attendance?CiaanId=${encodeURIComponent(CiaanId)}`,
          { headers: requestHeaders }
        );
        if (theoryRes.ok) {
      ...
      try {
        const practicalRes = await fetch(
          `${config.apiBaseUrl}/practical-attendance?CiaanId=${encodeURIComponent(CiaanId)}`,
          { headers: requestHeaders }
        );
        if (practicalRes.ok) {
      ...
      try {
        const tutorialRes = await fetch(
          `${config.attendance.tutorial}?CiaanId=${encodeURIComponent(CiaanId)}`,
          { headers: requestHeaders }
        );
        if (tutorialRes.ok) {
      ...
      try {
        const extraRes = await fetch(
          `${config.attendance.extraTheory}/Ciaan/${encodeURIComponent(CiaanId)}`,
          { headers: requestHeaders }
        );
        if (extraRes.ok) {
      ...
      <div className="attendance-sheet-landscape page-break-before" key={`${type}-${batchName || ""}-page-${pageIndex}`}>
        <div className="ledger-header-info">
          <div className="ledger-left">
            Class & Div. : <span className="ledger-value">{getClassAndDiv(CiaanData)}</span>
          </div>
  ...
  return (
    <div className="Ciaan-print-page">
      <div className="Ciaan-print-toolbar no-print">
        <button
      ...
      <div className="Ciaan-print-sheet">
        <section className="print-block front-page-block">
          ...
          <div className="course-diary-card">
            ...
            <div className="course-diary-body-center">
              ...
              <h3 className="Ciaan-name">CIAAN</h3>
              <p>Curriculum Implementation and Assessment Norms</p>
              <p>
                <strong>Academic Year:</strong> {CiaanData?.academicYear || "-"}
              </p>
              <p>
            ...
            <div className="diary-info-table">
              ...
              <div className="diary-row">
                ...
                <span className="value">{getClassAndDiv(CiaanData)}</span>
              </div>
              <div className="diary-row">
                <span className="label">Subject & Subject Code</span>
                <span className="value">{getSubjectAndCode(CiaanData)}</span>
              </div>
              <div className="diary-row">
                <span className="label">Department</span>
                <span className="value">{getDepartment(CiaanData)}</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="print-block page-break-before k3-format-section">
          ...
          <div className="k3-meta">
            ...
            <div><strong>Course and Code:</strong> {CiaanData?.courseCode || CiaanData?.class || "C05K-A"}</div>
            <div><strong>Subject and Code:</strong> {CiaanData?.subject?.name || "CLOUD COMPUTING"} ({CiaanData?.subject?.code || "315325"})</div>
            <div><strong>Name of Faculty:</strong> {formatUsername(localStorage.getItem("username") || "") || "Faculty"}</div>
            <div><strong>Division:</strong> {CiaanData?.division || "-"}</div>
          </div>
              ...
              <thead>
                ...
                <tr>
                  <th colSpan="3" className="k4-meta-left">
                    <div>
                      <strong>Academic Year:</strong> {CiaanData?.academicYear || "2025 - 2026"}
                    </div>
                  </th>
                ...
                <tr>
                  <th colSpan="2" className="k4-meta-left">
                    <strong>Course and Code:</strong> {CiaanData?.courseCode || CiaanData?.class || "C05K-A"}
                  </th>
        ...
        <section className="print-block page-break-before attendance-records-section">
          ...
          <div className="attendance-meta">
            <div><strong>Academic Year:</strong> {CiaanData?.academicYear || "2025 - 2026"}</div>
            <div><strong>Course/Semester:</strong> {CiaanData?.class || "-"} ({CiaanData?.division || "-"})</div>
            <div><strong>Subject and Code:</strong> {CiaanData?.subject?.name || "-"} ({CiaanData?.subject?.code || "-"})</div>
        ...
        {loading && (
          <p className="Ciaan-print-loading">Loading complete Ciaan content...</p>
        )}
      </div>
    </div>
  );
