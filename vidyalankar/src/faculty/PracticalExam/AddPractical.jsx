  ...
  const fetchDivisionsFromOfficePanel = async () => {
    try {
      ...
      if (Ciaans && Array.isArray(Ciaans)) {
        if (Ciaans.length === 0) {
          setDivisions([]);
          setError("No Ciaans found. Please create Ciaans first.");
          return;
        }

        let divisionList = [];
        const primaryCiaanId = Ciaans[0]?._id;

        if (primaryCiaanId) {
          const divisionsResponse = await fetch(
            `${config.apiBaseUrl}/practical-exams/divisions?batchId=${primaryCiaanId}`,
            {
        ...
        if (divisionList.length === 0) {
          setError("No divisions found for your Ciaans.");
        }

        setDivisions(divisionList.sort());
      } else {
        setError("Failed to load divisions from Ciaans");
      }
    } catch (err) {
