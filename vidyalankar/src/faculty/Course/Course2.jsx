...
const PracticalTable = () => {
  ...
  useEffect(() => {
    ...
    if (!resolvedCiaan) {
      ...
      if (stored) {
        try {
          ...
          console.error("Error parsing stored CiaanData", e);
        }
      }
    }
    ...
    if (qProgram && qClassName && qCourse) {
      ...
      setCourse(qCourse);
    } else if (resolvedCiaan) {
      // Otherwise extract from CiaanData
      ...
      let prog = extractValue(resolvedCiaan, "department", "dept", "departmentName", "Department", "DEPARTMENT", "branch", "Branch", "stream", "Stream") || "N/A";
      if (prog === "N/A" || typeof prog !== "string") {
        const nestedDept = resolvedCiaan?.academicInfo?.department || resolvedCiaan?.courseDetails?.department || resolvedCiaan?.details?.department || resolvedCiaan?.info?.department || resolvedCiaan?.subjectDetails?.department || resolvedCiaan?.class?.department || resolvedCiaan?.subject?.department;
        if (nestedDept) {
      ...
      let cls = extractValue(resolvedCiaan, "className", "class", "classname", "Class", "ClassName", "semester", "year") || "N/A";
      if (cls === "N/A") cls = resolvedCiaan?.classDetails?.className || resolvedCiaan?.class?.name || resolvedCiaan?.class || resolvedCiaan?.academicInfo?.class || "N/A";
      if (typeof cls === "object" && cls !== null) cls = cls.name || cls.label || "N/A";

      let crs = extractValue(resolvedCiaan, "subjectName", "subject", "subjectname", "Subject", "SubjectName", "title", "courseName", "name") || "N/A";
      if (crs === "N/A") crs = resolvedCiaan?.subjectDetails?.name || resolvedCiaan?.subject?.name || resolvedCiaan?.subject || resolvedCiaan?.courseInfo?.subject || "N/A";
