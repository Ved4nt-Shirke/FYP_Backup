...
export default function ViewPA3() {
  ...
  let CiaanData = locationData.CiaanData;
  if (!CiaanData) {
    try {
      const storedData = sessionStorage.getItem('viewPA_CiaanData');
      if (storedData) {
        CiaanData = JSON.parse(storedData);
      } else {
        CiaanData = {};
      }
    } catch (e) {
  ...
  // Debug logging
  console.log('ViewPA3 - Batch:', batch);
  console.log('ViewPA3 - CIAAN Data:', CiaanData);
  ...
  useEffect(() => {
    console.log('ViewPA3 useEffect - Checking data...');
    console.log('Batch:', batch);
    console.log('CIAAN Data:', CiaanData);
    console.log('CIAAN ID:', CiaanData?.CiaanId);

    if (batch) {
      console.log('ViewPA3 - Batch found, fetching assessment data...');
      fetchAssessmentData();
    } else {
      console.log('ViewPA3 - Missing batch data. Batch:', batch);
    ...
    //     sessionStorage.removeItem('viewPA_CiaanData');
    //   }
    // };
  }, [batch, CiaanData]);
    ...
    <>
      ...
      {!loading && !error && studentsData.length > 0 && (
        <div className="print-only-content" style={{ display: 'none' }}>
          ...
          {CiaanData && Object.keys(CiaanData).length > 0 && (
            <div style={{ border: '1px solid #000', padding: '10px', marginBottom: '15px', fontSize: '10pt' }}>
              <strong>CIAAN ID:</strong> {CiaanData.CiaanId || 'N/A'} |
              <strong> Subject:</strong> {CiaanData.subject?.name || 'N/A'} ({CiaanData.subject?.code || 'N/A'}) |
