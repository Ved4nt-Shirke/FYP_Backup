...
export default function StudentWiseSelect() {
  ...
  useEffect(() => {
    // Load division-aware batches when Ciaan is selected
    ...
    loadBatches();
  }, [CiaanData?.CiaanId, CiaanData?.division]);
  ...
  const handleProceed = () => {
    ...
    navigate('/studentwise-assess', {
      state: {
        ...
        // Be robust across different Ciaan schemas
        program: CiaanData?.department?.program || CiaanData?.department?.name || (typeof CiaanData?.department === 'string' ? CiaanData.department : ''),
        className: CiaanData?.class || '',
