...
export default function StudentAssessSetup() {
  ...
  const canProceed = selectedBatch && selectedStudent && CiaanData;

  const handleNext = () => {
    if (!canProceed) return;
    // Extract subject details needed to fetch experiments later
    const program = CiaanData?.department?.program || CiaanData?.department; // fallback
    const className = CiaanData?.class;
    const course = CiaanData?.subject?.name || CiaanData?.subject?.code;
      ...
      <div className="container" style={{ maxWidth: 800 }}>
        ...
        {CiaanData && (
          <div className="alert alert-info mt-3">
            <strong>CIAAN:</strong> {CiaanData.CiaanId} | <strong>Subject:</strong> {CiaanData.subject?.name} ({CiaanData.subject?.code}) | <strong>Division:</strong> {CiaanData.division}
          </div>
        )}
