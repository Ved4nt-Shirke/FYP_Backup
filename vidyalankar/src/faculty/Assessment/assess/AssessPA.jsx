...
export default function AssessPA() {
  ...
  const fetchExperiments = async () => {
    try {
      ...
      const storageCiaan = (() => {
        try { return JSON.parse(localStorage.getItem('CiaanData') || 'null'); } catch (_) { return null; }
      })();

      const effectiveProgram = (
        program ||
        CiaanData?.department?.program ||
        CiaanData?.department?.name ||
        CiaanData?.department?.label ||
        CiaanData?.department?.value ||
        (typeof CiaanData?.department === 'string' ? CiaanData.department : '') ||
        storageCiaan?.department?.program ||
        storageCiaan?.department?.name ||
        storageCiaan?.department?.label ||
        storageCiaan?.department?.value ||
        (typeof storageCiaan?.department === 'string' ? storageCiaan.department : '')
      )?.toString().trim();
      ...
      const effectiveCourse = (
        ...
        storageCiaan?.subject?.name ||
        storageCiaan?.subject?.code
      )?.toString().trim();
                ...
                <div className="text-center p-5 bg-white border rounded shadow-sm my-3">
                  ...
                  <button
                    ...
                    onClick={() => navigate("/assessment/edit-prog", {
                      state: { batch, CiaanData, program, className, course }
                    })}
                  >
