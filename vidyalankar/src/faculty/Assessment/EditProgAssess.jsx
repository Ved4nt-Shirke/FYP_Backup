...
function EditProgAssess() {
  ...
  const [CiaanData, setCiaanData] = useState(stateData.CiaanData || null);
  ...
  useEffect(() => {
    if (!CiaanData) {
      try {
        const stored = localStorage.getItem("CiaanData");
        if (stored) {
          setCiaanData(JSON.parse(stored));
        }
      } catch (e) {
        console.error("Error reading CiaanData in EditProgAssess:", e);
      }
    }
  }, [CiaanData]);
