...
export default function EditExperimentList() {
  ...
  const { batch, CiaanData, assessedExperiments: passedExperiments } = location.state || {};
  ...
  const fetchAssessedExperiments = async () => {
    try {
      ...
      const response = await fetch(`${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5000").replace(/\/api$/, "")}/api/assessments/assessed-experiments?batch=${batch}`);
      ...
      <div className="container mt-4">
        ...
        {batch && CiaanData && (
          <div className="alert alert-info">
            ...
            <strong>Division:</strong> {CiaanData.division || "N/A"} <br />
