...
export default function PTMarksEntry() {
  ...
  const { Ciaan, subject, config } = location.state || {};
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // Fetch Ciaan Students
        const studentsRes = await fetch(
          `${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5000").replace(/\/api$/, "")}/api/pt-microproject/new/students/${Ciaan.CiaanId}`,
          {
            headers: {
        ...
        const marksRes = await fetch(
          `${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5000").replace(/\/api$/, "")}/api/pt-microproject/new/marks/${Ciaan.CiaanId}/${subjectId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `PT_Marks_${subject.code || "Subject"}_Ciaan_${Ciaan.CiaanId}.csv`
    );
  ...
  if (!Ciaan || !subject || !config) return null;
      ...
      <div className="container-fluid py-5 px-md-5">
        ...
        <div className="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
          <div className="d-flex gap-2">
            ...
            <button
              onClick={() => navigate("/pt-microproject/configuration", { state: { Ciaan, subject, config } })}
          ...
          <div className="row align-items-center">
            <div className="col-md-8">
              ...
              <p className="mb-0 text-white-50">
                Course: {Ciaan.department?.name || "Computer Engineering"} &bull; Sem {Ciaan.semester} &bull; Division {Ciaan.division} &bull; AY {Ciaan.academicYear}
              </p>
            </div>
                            ...
                            <td key={comp.componentName}>
                              <div className="mx-auto" style={{ maxWidth: "100px" }}>
                                <input
                                  ...
                                  className={`form-control text-center rounded-3 shadow-sm ${isInvalid ? "is-invalid border-danger" : "border-0 bg-light"
                                    }`}
                                  style={{
                                    fontWeight: "600",
