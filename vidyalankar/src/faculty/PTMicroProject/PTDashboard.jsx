...
export default function PTDashboard() {
  ...
  useEffect(() => {
    const fetchCiaans = async () => {
      try {
        ...
        // Filter out Ciaans if they are not array (in case of error response)
  ...
  const handleCardSelect = (Ciaan) => {
    setSelectedCard(selectedCard?._id === Ciaan._id ? null : Ciaan);
  };

  const handleSubjectSelect = async (Ciaan, subject) => {
    setCheckingConfig(true);
    try {
      const token = localStorage.getItem("token");
      const subjectId = subject._id || subject.subjectId || Ciaan.subject?._id;
      ...
      const configRes = await fetch(
        `${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5000").replace(/\/api$/, "")}/api/pt-microproject/new/config/${Ciaan.CiaanId}/${subjectId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      <div className="container-fluid py-4 px-md-5">
        ...
        <div className="row">
          ...
          {loading ? (
            ...
            CiaanList.map((Ciaan) => {
              const deptName = Ciaan.department?.name || Ciaan.department || "N/A";
              const isSelected = selectedCard?._id === Ciaan._id;

              return (
                <div key={Ciaan._id} className="col-md-6 col-lg-4 mb-4">
                  <div
                    className={`card h-100 border-0 shadow-sm rounded-4 position-relative overflow-hidden cursor-pointer transition-all ${isSelected ? "border-start border-success border-5 shadow" : ""
                      }`}
                    style={{
                      cursor: "pointer",
                    ...
                    onClick={() => handleCardSelect(Ciaan)}
                  >
                    <div className="card-body p-4">
                      ...
                      <p className="text-muted small mb-3">
                        Semester {Ciaan.semester} &bull; Division {Ciaan.division}
                      </p>

                      <div className="p-3 bg-light rounded-3 mb-2">
                        <span className="text-muted small d-block mb-1">Subject</span>
                        <strong className="text-dark">{Ciaan.subject?.name || "Unknown Subject"}</strong>
                        <span className="text-secondary small d-block mt-1">Code: {Ciaan.subject?.code || "N/A"}</span>
                      </div>
                      ...
                      {isSelected && (
                        <div className="mt-4 pt-3 border-top">
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleSubjectSelect(Ciaan, Ciaan.subject);
                            }}
