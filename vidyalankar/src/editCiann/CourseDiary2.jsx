...
import logoImage from '../assets/logo.png'; // VP logo (green)
import vitLogo from '../assets/VIT.png'; // VIT logo (blue)
import vsitLogo from '../assets/VSIT-1.png'; // VSIT logo (red)
...
const CourseDiary2 = () => {
  ...
  // Get current theme/college from localStorage
  const [currentCollege, setCurrentCollege] = useState(localStorage.getItem('college') || 'VP');

  // Function to get the appropriate logo based on theme
  const getLogo = () => {
    switch (currentCollege) {
      case 'VSIT':
        return vsitLogo; // Red theme
      case 'VIT':
        return vitLogo; // Blue theme
      case 'VP':
      default:
        return logoImage; // Green theme (default)
    }
  };

  // Function to get institute name based on theme
  const getInstituteName = () => {
    switch (currentCollege) {
      case 'VSIT':
        return 'VSIT - Vidyalankar School of Information Technology';
      case 'VIT':
        return 'VIT - Vidyalankar Institute of Technology';
      case 'VP':
      default:
        return 'VP - Vidyalankar Polytechnic';
    }
  };
  ...
  const styles = {
    ...
    infoValue: {
      ...
      display: isMobile ? 'block' : 'table-cell',
      padding: '12px 15px', verticalAlign: 'middle', boxSizing: 'border-color'
    },
  ...
  return (
    <div style={styles.layout}>
      <Header onMenuToggle={() => setIsSidebarVisible(v => !v)} onSecondaryMenuToggle={() => setIsSecondarySidebarVisible(v => !v)}/>
          ...
          <div style={styles.container}>
            <header style={styles.headerWrapper}>
              <div style={{ flexShrink: 0 }}>
                <img src={getLogo()} alt={`${currentCollege} Logo`} style={styles.logo} />
              </div>
