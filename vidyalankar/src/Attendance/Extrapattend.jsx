...
const StudentAttendancePage = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      showErrorAlert("Invalid attendance data. Please go back and fill the form again.");
      return;
    }

    if (!meta || !meta.batch) {
      console.error("Missing batch in meta data:", meta);
      showErrorAlert("Missing batch information. Please go back and select a batch.");
      return;
    }

    console.log('Fetching students for batch:', meta.batch);
    const apiUrl = `http://localhost:5000/api/students?batch=${meta.batch}`;
    ...
    axios.get(apiUrl)
      ...
      .then(fallbackRes => {
        if (fallbackRes) {
          ...
          const filteredStudents = fallbackRes.data.filter(student => student.batch === meta.batch);
          console.log('Frontend filtered students:', filteredStudents);
          
          if (filteredStudents.length === 0) {
            alert(`No students found for batch ${meta.batch}. Please check if students are added to this batch.`);
            setStudents([]);
            return;
          }
      ...
      .catch(err => {
        ...
        if (err.response?.status === 404) {
          ...
          alert("Cannot connect to backend server. Please check if the server is running on port 5000.");
        } else {
  ...
  const handleSubmitAttendance = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/extra-pract', payload);
    ...
    <div className="timetable-main-content">
      <div className="theory-attendance-container">
        <div className="header-row">
          <h3>Student Extra Practical Attendance</h3>
        </div>
