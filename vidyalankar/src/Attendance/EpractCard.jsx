...
import "../components/EditCiann.css"; // Reusing the same CSS

const ExtraPracticalCiannCards = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        showErrorAlert("Failed to fetch CIANNs: " + err.message);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="edit-ciann-page">
        <div className="edit-ciann-header">
          <h2 className="text-center py-2 bg-success text-white">
            Extra Practical Attendance - Select CIAAN
          </h2>
        </div>
          ...
          {loading ? (
            ...
            ciannDataList.map((ciannData) => (
              <div
                ...
                onClick={() =>
                  navigate("/extra-practical", { state: { ciannData } })
                }
              >
...
export default ExtraPracticalCiannCards;
