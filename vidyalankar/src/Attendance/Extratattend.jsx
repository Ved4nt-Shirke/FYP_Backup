...
const StudentAttendance = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const attendanceResponse = await fetch(`http://localhost:5000/api/extra-attendance/${id}`);
        ...
        // Map DB fields (rollNo, studentName) to component fields (rollId, name)
  ...
  const handleCheckboxChange = async (rollId, name, isChecked) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/extra-attendance/${id}/mark`, {
        method: "PATCH",
  ...
  const handleFinalSubmit = async () => {
    try {
      ...
      if (failed.length > 0) {
        ...
        alert(`Warning: ${failed.length} students could not be marked. Please try again.`);
      } else {
      ...
      // navigate('/extra-theory-ciann-cards');
    } catch (error) {
  ...
  return (
    <div className="timetable-main-content">
      <div className="theory-attendance-container">
        <div className="header-row">
          <h3>Student Extra Theory Attendance</h3>
        </div>
            ...
            {showDropdown && (
              <div className="dropdown-menu">
                <label className="dropdown-label">
                  <input type="checkbox" checked={visibleColumns.rollId} onChange={() => toggleColumn('rollId')} />
                  Roll ID
                </label>
                <label className="dropdown-label">
                ...
                <label className="dropdown-label">
                  <input type="checkbox" checked={visibleColumns.mark} onChange={() => toggleColumn('mark')} />
                  Mark
                </label>
              </div>
            )}
        ...
        <div className="table-container">
          <table>
            {showHeaders && (
              <thead>
                <tr>
                  {visibleColumns.rollId && <th>Roll ID</th>}
