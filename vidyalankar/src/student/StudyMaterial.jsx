...
const StudyMaterial = () => {
  ...
  const renderMaterialCard = (material) => {
    const ytId = material.resourceType === "link" && material.externalUrl ? getYoutubeId(material.externalUrl) : null;
    return (
      <div key={material._id} className="material-explorer-card">
        <div className="material-card-icon-area" onClick={() => handleOpenMaterial(material)}>
          {material.thumbnailPath ? (
            ...
            <img src={`https://img.youtube.com/vi/${ytId}/0.jpg`} alt="" className="material-card-thumbnail" />
          ) : material.fileMimeType?.startsWith("video/") || (material.category === "Video Lectures" && material.resourceType === "file") ? (
            <video src={getVideoSrc(material.downloadUrl)} preload="metadata" className="material-card-thumbnail" style={{ objectFit: "cover", width: "100%", height: "100%" }} />
          ) : (
  ...
  const renderDocsTabContent = () => {
    const docCategories = ["Notes", "PPT / Presentation", "Lab Manual", "Practical Files", "Reference PDFs", "E-books / EPA", "EPA"];
        ...
        <div className="slider-wrapper recent-tab-slider">
          ...
          <div className="horizontal-slider">
            {continueWatchingSubj.map((item) => {
              const ytId = item.resourceType === "link" && item.externalUrl ? getYoutubeId(item.externalUrl) : null;
              return (
                <div key={item._id} className="slider-item-card glass-card" onClick={() => handleOpenMaterial(item)}>
                  <div className="slider-video-thumbnail">
                    {item.thumbnailPath ? (
                      ...
                      <img src={`https://img.youtube.com/vi/${ytId}/0.jpg`} alt={item.title} />
                    ) : item.fileMimeType?.startsWith("video/") || (item.category === "Video Lectures" && item.resourceType === "file") ? (
                      <video src={getVideoSrc(item.downloadUrl)} preload="metadata" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    ) : (
            ...
            <div className="player-body">
              {activeVideo.resourceType === "link" && activeVideo.externalUrl?.includes("youtube.com") ? (
                ...
                <video
                  ...
                  controls
                  autoPlay
                  src={getVideoSrc(activeVideo.downloadUrl)}
                ></video>
              )}
            </div>
          </div>
