...
const StudentTimetable = () => {
  ...
  const [viewMode, setViewMode] = useState("schedule"); // schedule, grid, official
  ...
  const { ongoing, activeDayName } = getOngoingLecture();
      ...
      <div className="timetable-main-panel-full">
          ...
          <div className="action-toolbar">
            ...
            {visible && timetable && (
              <div className="view-mode-selector">
                <button
                  className={`mode-btn ${viewMode === "schedule" ? "active" : ""}`}
                  onClick={() => setViewMode("schedule")}
                >
                  <i className="bi bi-list-task"></i> Day View
                </button>
                <button
                  className={`mode-btn ${viewMode === "grid" ? "active" : ""}`}
                  onClick={() => setViewMode("grid")}
                >
                  <i className="bi bi-grid-3x3-gap"></i> Week Grid
                </button>
                <button
                  className={`mode-btn ${viewMode === "official" ? "active" : ""}`}
                  onClick={() => setViewMode("official")}
                >
                  <i className="bi bi-file-earmark-pdf"></i> Official PDF
                </button>
              </div>
            )}
          </div>
        ...
        {!visible || !timetable ? (
          ...
          <div className="timetable-view-content">
            ...
            {viewMode === "schedule" && (
              <>
                ...
                <div className="day-navigation">
                  {DAYS.map((day) => {
                    const lectureCount = MOCK_LECTURES[day]?.length || 0;
                    return (
                      <button
                        key={day}
                      ...
                      >
                        ...
                        {lectureCount > 0 && <span className="lecture-badge">{lectureCount}</span>}
                      </button>
                    );
                  })}
                </div>

                {/* Daily Schedule List (Full-Width cards) */}
                <div className="lectures-schedule-list">
                  {(!MOCK_LECTURES[activeDay] || MOCK_LECTURES[activeDay].length === 0) ? (
                    <div className="no-lectures-box">
                      <i className="bi bi-clipboard-x"></i>
                      <p>No lectures scheduled for {activeDay}.</p>
                    </div>
                  ) : (
                    MOCK_LECTURES[activeDay].map((lec, idx) => {
                      const isLecOngoing = activeDay === activeDayName && ongoing && ongoing.subject === lec.subject && ongoing.time === lec.time;
                      return (
                        <div
                          key={idx}
                          className={`lecture-schedule-card ${isLecOngoing ? "ongoing-card-glowing" : ""}`}
                          style={{ "--subject-color": lec.color }}
                        >
                          <div className="card-accent-bar" style={{ background: lec.color }}></div>
                          <div className="lecture-time-column">
                            <span className="lec-time">{lec.time.split(" - ")[0]}</span>
                            <span className="lec-duration">1 Hour Lecture</span>
                          </div>
                          
                          <div className="lecture-details-column">
                            <div className="subject-row">
                              <h3>{lec.subject}</h3>
                              <span className="subject-code-badge">{lec.code}</span>
                              {isLecOngoing && (
                                <span className="live-status-badge">
                                  <span className="live-dot-pulse"></span> ONGOING NOW
                                </span>
                              )}
                            </div>
                            ...
                            <div className="meta-row">
                              <span className="meta-item">
                                <i className="bi bi-person-workspace"></i> Faculty Coordinator: <strong>{lec.faculty}</strong>
                              </span>
                              <span className="meta-item">
                                <i className="bi bi-geo-alt"></i> Room Location: <strong>{lec.room}</strong>
                              </span>
                              <span className="meta-item">
                                <i className="bi bi-clock"></i> Time Window: <strong>{lec.time}</strong>
                              </span>
                            </div>
                          </div>
                        </div>
            ...
            {/* View Mode: Week Grid */}
            {viewMode === "grid" && (
              <div className="timetable-week-grid-container">
                  ...
                  <table className="timetable-grid-table">
                    ...
                    <tbody>
                      {TIME_SLOTS.map((slot, sIdx) => (
                        <tr key={sIdx}>
                          <td className="time-label sticky-col">
                            <strong>{slot.label}</strong>
                          </td>
                          {DAYS.map((day, dIdx) => {
                            const lec = MOCK_LECTURES[day]?.find(l => {
                              const [lStart] = l.time.split(" - ");
                              return lStart === slot.start;
                            });

                            if (!lec) {
                              return <td key={dIdx} className="empty-grid-cell">Free Window</td>;
                            }

                            return (
                              <td key={dIdx} className="lecture-grid-cell" style={{ "--grid-accent": lec.color }}>
                                <div className="cell-content">
                                  <span className="grid-sub-name">{lec.subject}</span>
