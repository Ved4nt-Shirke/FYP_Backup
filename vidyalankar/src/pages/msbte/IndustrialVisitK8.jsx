...
const IndustrialVisitK8 = () => {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      const res = await axios.get("/msbte/industrial-visit/k8");
      if (res.data?.success && Array.isArray(res.data.data)) {
        setRecords(res.data.data);
      }
    } catch (err) {
      console.error("Failed to fetch K8 records:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this industrial visit record?")) {
      return;
    }

    try {
      const res = await axios.delete(`/msbte/industrial-visit/k8/${id}`);
      if (res.data?.success) {
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          ...
          <button
            ...
            onClick={() => navigate("/msbte/industrial-visit/k8/edit")}
            style={{
              background: "#4f46e5",
              color: "#fff",
              padding: "10px 20px",
          ...
          >
            <i className="bi bi-plus-lg"></i> Create New K8 Report
          </button>
        </div>
        ...
        >
          ...
          {loading ? (
            ...
            <div className="text-center py-5 text-muted">
              ...
              <p className="mt-3">No saved industrial visit reports found.</p>
              <button
...
export default IndustrialVisitK8;
