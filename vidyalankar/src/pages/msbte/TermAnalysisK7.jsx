...
const TermAnalysisK7 = () => {
  ...
  const handleCiaanChangeInForm = async (e) => {
    const cid = e.target.value;
    setSelectedCiaanId(cid);
    if (!cid) return;

    const CiaanObj = Ciaans.find((c) => String(c.CiaanId) === String(cid));
    if (CiaanObj) {
      setAcademicYear(CiaanObj.academicYear || "");
      setProgramme(CiaanObj.department?.name || CiaanObj.department || "");
      setDivision(CiaanObj.division || "");
      setSemester(CiaanObj.semester || "");
      setCourseCode(CiaanObj.subject?.code || "");
      setCourseName(CiaanObj.subject?.name || "");

      // Fetch and auto-calculate stats
      const deptId = CiaanObj.department?._id || CiaanObj.department;
      const sem = CiaanObj.semester;
      const divisionName = CiaanObj.division;
      const courseCode = CiaanObj.subject?.code;

      try {
        const divId = await getDivisionId(deptId, sem, divisionName);
        if (divId) {
          const res = await axios.get(
            `/msbte/k7/populate?academicYear=${CiaanObj.academicYear}&semester=${sem}&departmentId=${deptId}&divisionId=${divId}&CiaanId=${cid}`
          );
  ...
  const openEditModal = async (rec) => {
    ...
    try {
      const CiaanObj = Ciaans.find((c) => String(c.CiaanId) === String(rec.CiaanId));
      if (CiaanObj) {
        const deptId = CiaanObj.department?._id || CiaanObj.department;
        const sem = CiaanObj.semester;
        const divisionName = CiaanObj.division;
        ...
        if (divId) {
          const res = await axios.get(
            `/msbte/k7/populate?academicYear=${CiaanObj.academicYear}&semester=${sem}&departmentId=${deptId}&divisionId=${divId}&CiaanId=${rec.CiaanId}`
          );
          if (res.data?.success && Array.isArray(res.data.studentMarks)) {
            const calculatedHeads = calculateHeadsFromMarks(res.data.studentMarks, courseCode);
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="row g-3 mb-4">
                <div className="col-md-6">
                  ...
                  <select
                    ...
                    onChange={handleCiaanChangeInForm}
                    required
                    disabled={!!recordId}
                  >
              ...
              <div className="d-flex justify-content-end gap-3">
                ...
                <button
                  type="submit"
                  className="btn btn-primary"
                  disabled={saving || !selectedCiaanId}
