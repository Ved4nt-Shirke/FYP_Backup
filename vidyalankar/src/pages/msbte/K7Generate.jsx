...
const K7Generate = () => {
  ...
  // ── Ciaan manual input state ────────────────────────────────────────────────
  const [CiaanId, setCiaanId] = useState("");
  const [CiaanData, setCiaanData] = useState(null);

  const handleLoadCiaan = async (cid, urlDivId = "") => {
    if (!cid) return;
    setLoading(true);
    try {
      const res = await axios.get(`/Ciaans/${cid}`);
      if (res.data) {
        const Ciaan = res.data;
        setCiaanData(Ciaan);
        ...
        alert("Ciaan not found or access denied.");
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const urlCiaanId = p.get("CiaanId") || "";
    ...
    if (urlCiaanId) {
      setCiaanId(urlCiaanId);
      handleLoadCiaan(urlCiaanId, divId);
  ...
  const init = async (ay, sem, deptId, divId, divName, urlCiaanId, urlCourseCode) => {
    setLoading(true);
    try {
      // 1. department name
      ...
      if (k7Res.data?.success && k7Res.data.data) {
        ...
        if (urlCourseCode && !hasCourse) {
          await autoPopulate(ay, sem, deptId, targetDivId, urlCiaanId, urlCourseCode);
        } else {
  ...
  const autoPopulate = async (ay, sem, deptId, divId, urlCiaanId, urlCourseCode) => {
    try {
      const res = await axios.get(
        `/msbte/k7/populate?academicYear=${ay}&semester=${sem}&departmentId=${deptId}&divisionId=${divId}${urlCiaanId ? `&CiaanId=${urlCiaanId}` : ''}`
      );
