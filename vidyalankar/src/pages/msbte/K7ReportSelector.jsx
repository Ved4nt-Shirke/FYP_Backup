...
const K7ReportSelector = () => {
  ...
  // Handle Ciaan change with auto-population of cascades
  const handleCiaanChange = async (CiaanId) => {
    setSelectedCiaanId(CiaanId);
    if (!CiaanId) {
      setSelectedDeptId("");
      setSelectedCourseId("");
      setSelectedDivId("");
    ...
    const Ciaan = Ciaans.find((c) => String(c.CiaanId) === String(CiaanId));
    if (!Ciaan) return;

    setAcademicYear(Ciaan.academicYear);
    ...
    try {
      ...
      if (courseRes.data?.success && Array.isArray(courseRes.data.courses)) {
        ...
        const matchedCourse = courseRes.data.courses.find(
          (c) => Number(c.semester) === Number(Ciaan.semester)
        );
        if (matchedCourse) {
          ...
          if (divRes.data?.success && Array.isArray(divRes.data.divisions)) {
            ...
            const matchedDiv = divRes.data.divisions.find(
              (d) => String(d.name).toLowerCase() === String(Ciaan.division).toLowerCase()
            );
            if (matchedDiv) {
      ...
      console.error("Failed to auto-populate parameters from Ciaan:", err);
    } finally {
      setLoadingCourses(false);
      setLoadingDivisions(false);
    }
  ...
  const handleAction = (type) => {
    ...
    if (selectedCiaanId) {
      const Ciaan = Ciaans.find((c) => String(c.CiaanId) === String(selectedCiaanId));
      if (Ciaan) {
        queryStr += `&CiaanId=${Ciaan.CiaanId}&courseCode=${Ciaan.subject?.code}`;
      }
    }
              ...
              <div className="col-12">
                ...
                {loadingCiaans ? (
                  ...
                  <select
                    ...
                    onChange={(e) => handleCiaanChange(e.target.value)}
                    style={{ borderColor: "#4f46e5", borderWidth: "2px", fontWeight: "600" }}
                  >
                    <option value="">-- Choose Ciaan --</option>
                    {filteredCiaans.map((c) => (
                      <option key={c._id} value={c.CiaanId}>
                        Ciaan {c.CiaanId} - {c.subject?.name} ({c.subject?.code}) | Sem {c.semester} - Div {c.division} | {c.academicYear}
                      </option>
                    ))}
                  </select>
                )}
